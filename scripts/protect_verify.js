 ...
 *
 *  用法:
 *    node scripts/protect_verify.js encrypt   -- 加密并提交.enc
 *    node scripts/protect_verify.js decrypt   -- 解密（构建时调用）
 *
 *  密钥来源（优先级）:
 *    1. 环境变量 POLARIS_VERIFY_KEY
 *    2. 本地文件 .polaris_key（gitignored）
 *    3. 从 secrets.js vault 自动衍生
 *
 *  .enc 文件可公开提交到 GitHub。
 *  没有密钥的人拿到 .enc 也解不开。
 * ═══════════════════════════════════════════════════════════
...
const VERIFY_SRC = path.join(ROOT, 'electron', 'services', 'verification_engine.js');
const VERIFY_ENC = path.join(ROOT, 'electron', 'services', 'verification_engine.js.enc');
const KEY_FILE = path.join(ROOT, '.polaris_key');
...
function getKey() {
  ...
  try {
    const vaultPath = path.join(ROOT, 'electron', 'services', 'secrets.js');
