...
const App:React.FC=()=>{
  ...
  const SidebarContent=()=>{const[tab,setTab]=useState('chats');
  const statusIcon=(s:string)=>s==='running'?'●':s==='done'?'✓':s==='error'?'✗':'○';
  const statusColor=(s:string)=>s==='running'?'var(--accent)':s==='done'?'var(--success)':s==='error'?'var(--error)':'var(--text3)';
  return(<div className="side"><div className="side-hd"><span className="side-logo">POLARIS</span><button className="side-cl"onClick={()=>d(toggleSidebar())}>&times;</button></div><div className="side-tabs"><button className={'side-tb'+(tab==='chats'?' on':'')}onClick={()=>setTab('chats')}>Sessions</button><button className={'side-tb'+(tab==='exec'?' on':'')}onClick={()=>setTab('exec')}>Exec{execLog.filter(e=>e.status==='running').length>0&&<span style={{color:'var(--accent)',marginLeft:4}}>●</span>}</button><button className={'side-tb'+(tab==='tasks'?' on':'')}onClick={()=>setTab('tasks')}>Tasks{todoSteps.some(s=>s.status==='running')&&<span style={{color:'var(--amber)',marginLeft:4}}>●</span>}</button><button className={'side-tb'+(tab==='tools'?' on':'')}onClick={()=>setTab('tools')}>Tools</button></div><button className="side-new"onClick={()=>d(ns())}>+ New Session</button>
{tab==='exec'&&<div className='side-section'><div className='side-section-title'>Execution</div><div style={{display:'flex',flexDirection:'column',gap:1,fontSize:10,fontFamily:'var(--fm)',maxHeight:'calc(100vh - 200px)',overflowY:'auto'}}>{execLog.length===0?<span style={{color:'var(--text3)',padding:4}}>等待工具调用...</span>:execLog.slice().reverse().map(e=><div key={e.id} style={{padding:'4px 6px',borderRadius:4,display:'flex',gap:6,alignItems:'flex-start',lineHeight:1.5}}><span style={{color:statusColor(e.status),fontSize:10,flexShrink:0,marginTop:1}}>{statusIcon(e.status)}</span><span style={{flex:1}}><span style={{color:'var(--text2)',fontWeight:500}}>{e.tool}</span><span style={{color:'var(--text3)',marginLeft:4}}>{e.time}</span><br/><span style={{color:'var(--text3)',wordBreak:'break-all'}}>{e.detail}</span></span></div>)}</div></div>}
    
{tab==='tasks'&&<div className="side-section"><div className="side-section-title">Task Pipeline</div><div style={{display:"flex",flexDirection:"column",gap:2,fontSize:11}}>{todoSteps.length===0?<span style={{color:"var(--text3)",padding:4}}>暂无任务...</span>:todoSteps.map(t=><div key={t.id} style={{padding:"6px 8px",borderRadius:4,display:"flex",gap:8,alignItems:"center"}}><span style={{color:t.status==="done"?"var(--green)":t.status==="running"?"var(--amber)":"var(--text3)",fontSize:12}}>{t.status==="done"?"u2713":t.status==="running"?"u25cf":"u25cb"}</span><span style={{color:t.status==="running"?"var(--ink)":"var(--ink2)",fontWeight:t.status==="running"?600:400}}>{t.label}</span></div>)}</div></div>}
