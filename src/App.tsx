  ...
  return(
    <div className="w-[272px] shrink-0 bg-card border-r flex flex-col overflow-hidden">
      ...
      <div className="flex px-3 gap-0.5">
        {['chats','exec','tasks','tools'].map((t:any)=>{let dot=null;if(t==='exec'&&execLog.some((e:any)=>e.status==='running'))dot=<span className="text-primary ml-1">●</span>;if(t==='tasks'&&todoSteps.some((s:any)=>s.status==='running'))dot=<span className="text-amber-500 ml-1">●</span>;const labels={chats:'会话',exec:'Exec',tasks:'Tasks',tools:'工具'};return<button key={t} className={'flex-1 py-2 text-[11px] font-medium rounded-md transition-colors '+(tab===t?'bg-primary/10 text-primary':'text-muted-foreground hover:text-foreground')}onClick={()=>setTab(t)}>{labels[t]}{dot}</button>})}
      </div>
